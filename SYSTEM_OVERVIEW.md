...
品川から札幌へ:
  - 余剰: 2名
  - 不足: 3名
  - 移動人数: min(2, 3, 2) = 2名  ← 最大2名まで制限
  → 実際は1名に調整（他拠点との配分を考慮）

盛岡から札幌へ:
  - 余剰: 3名
  - 不足: 3名（品川から1名移動後は2名不足）
  - 移動人数: min(3, 2, 2) = 2名
```
...
品川から1名選定:
  ...
  候補: [上野由香利(Lv4), 佐藤花子(Lv3)]
  → ランダムまたはシャッフル後に1名選択
  → 上野由香利さん

盛岡から2名選定:
  キー: (盛岡, SS, 新SS(W), OCR対象, エントリ1)
  候補: [高山麻由子(Lv4), 竹下朱美(Lv3), 田中美咲(Lv3)]
  → シャッフル後に2名選択
  → 高山麻由子さん、竹下朱美さん
```
...
1. **不足と余剰のマッチング**
   - 同じ工程（エントリ1）でマッチング
   - 同じ業務階層（SS > 新SS(W) > OCR対象）でマッチング

2. **移動人数の制限**
   ```python
   transfer_count = min(surplus, needed, 2)  # 最大2名
   ```
   - 余剰人数を超えない
   - 不足人数を超えない
   - 1拠点から最大2名まで
...
3. **オペレータ選定**
   - スキルレベルは考慮するが、基本はランダム
   - 4階層（拠点、業務、OCR区分、工程）で完全一致
...
1. 「札幌が3名不足」というデータをもらう
2. 「品川と盛岡に余剰がある」というデータをもらう
3. 「じゃあ品川から1名、盛岡から2名を札幌に移動しよう」と決める
4. その拠点のオペレータリストから実名を選ぶ
5. 影響を計算（配置変更2件 → 生産性+20%, 遅延-30分）
