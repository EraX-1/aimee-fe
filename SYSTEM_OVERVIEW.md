...
```
残タスク数: 120件
残り時間: 20分（16:00 - 15:40）

必要処理速度 = 120件 ÷ 20分 = 6件/分
```

**計算 2: 現在の処理速度**
```
現在の処理速度 = 5件/分（過去の実績から）
```
...
```
平均処理速度 = 0.3件/分/人（実績値）
必要人数 = 1件/分 ÷ 0.3件/分/人 ≈ 3名
```

**候補選定**:
```
1. 上野由香利 (品川) - スキルLv4, 経験48ヶ月 → 選択
2. 高山麻由子 (盛岡) - スキルLv4, 経験36ヶ月 → 選択
3. 竹下朱美   (盛岡) - スキルLv3, 経験24ヶ月 → 選択
```
...
```json
{
  ...
  "reason": "管理者基準では納期20分前は3名移動が推奨されています。"
}
```

**解説**:
- Pythonロジックで計算（LLM不使用）
- スキルレベル降順、経験年数降順で候補選定
- RAG検索結果を理由に含める
